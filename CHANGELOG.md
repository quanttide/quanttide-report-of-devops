...
### Added
- 添加架构决策记录（ADR），包括 devops-code boundary、git CLI 使用、metrics、codegen、offline status 等主题
- 新增实验报告归档（git-exp / bench-coverage）、DevOps 测试方法论及测试阶段工作纪要
- 新增 contract.md，描述四维架构实现对照
- 新增 release modeling report（specification）及更新后的 modeling report（含 Entry/Record 拆分）

### Changed
- 重命名文档目录 architecture/ 为 adr/
- 调整 git 使用相关文档名称（git-network-use-cli → git-library-strategy → git-library-usage）
